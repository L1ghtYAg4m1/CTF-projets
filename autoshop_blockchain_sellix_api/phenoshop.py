...
def update_user_balance(user_id, crypto_type, amount):
    ...
    sol_rate = conversion_rates['SOL']
    eth_rate = conversion_rates['ETH']
    ltc_rate = conversion_rates['LTC']
    
    try:
        if crypto_type == 'SOL':
            euro_amount = amount * sol_rate
            c.execute('''
            ...
            ''', (user_id, amount, euro_amount, euro_amount))
        elif crypto_type == 'ETH':
            euro_amount = amount * eth_rate
            c.execute('''
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for row in rows:
            ...
            params = {
                ...
                'endblock': 99999999,
                'sort': 'asc',
                'apikey': 'SJPMB1MAXXPNSZGUQXCTEIGFTCMZKJR8Z8'
            }
            try:
                logger.info(f"Checking ETH deposits for address {eth_address}")
                async with session.get(url, params=params, ssl=False) as response:
                    result = await response.json()
                    logger.info(f"ETH deposit check result: {result}")
                    if 'result' in result and isinstance(result['result'], list):
                        transactions = result['result']
                        destination_address = '0xe6D8C450b568eA4c6D5Bc6Fa845251ee96BB350e'  # Replace with actual destination address
                        for tx in transactions:
                            # Check if the transaction is an incoming one
                            if tx['to'].lower() == destination_address.lower():
                                deposit_amount = int(tx['value']) / 1e18  # Convert Wei to Ether
                                tx_hash = tx['hash']
...
async def check_sol_deposit(application: Application):
    async with aiohttp.ClientSession() as session:
        url = 'https://api.solana.fm/v0/address'
        conn = get_db_connection()
        c = conn.cursor()
        ...
        for row in rows:
            ...
            try:
                ...
                async with session.get(f"{url}/{sol_address}/transactions", ssl=False) as response:
                    result = await response.json()
                    ...
                    if 'transactions' in result:
                        ...
                        for tx in transactions:
                            if tx['destination'] == destination_address:
                                deposit_amount = float(tx['amount']) / 1e9  # Adjust the conversion factor as needed
                                tx_hash = tx['txid']
                                # Check if this transaction has already been processed
                                if not transaction_processed(tx_hash):
...
async def monitor_deposits_periodically(application: Application):
    ...
    while True:
        ...
        await asyncio.sleep(20)  # Wait for 20 seconds before running the next check
