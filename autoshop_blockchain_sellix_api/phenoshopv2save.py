...
async def handle_profile(update: Update, context: CallbackContext) -> None:
    ...
    await update.callback_query.edit_message_text(profile_text, parse_mode=ParseMode.HTML, reply_markup=reply_markup)
...
async def handle_deposit_crypto(update: Update, context: CallbackContext) -> None:
    text = "💰 Choisissez la crypto pour le dépôt :"
    keyboard = [
...
async def main() -> None:
    ...
    # Add handlers for commands and button callbacks
    application.add_handler(CommandHandler('start', start))
    application.add_handler(CallbackQueryHandler(button))

    # Start updating the rates periodically
